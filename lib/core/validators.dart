...
class V {
  ...
  static Validator isbn() {
    return (v) {
      if (v == null || v.trim().isEmpty) return 'Введите ISBN';
      final clean = v.replaceAll(RegExp(r'[-\s]'), '');
      if (clean.length != 13 && clean.length != 10) return 'ISBN: 10 или 13 цифр';
      if (!RegExp(r'^\d+$').hasMatch(clean)) return 'ISBN: только цифры и дефисы';
      return null;
    };
  }
