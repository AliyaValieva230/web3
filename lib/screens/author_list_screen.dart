...
class _AuthorListScreenState extends State<AuthorListScreen> {
  ...
  Widget _buildBody(BuildContext context, AuthorListNotifier notifier) {
    ...
    if (notifier.result.items.isEmpty) return const Center(child: Text('Авторы не найдены'));
    ...
    if (isCompact) {
      return EntityCardList<Author>(
        ...
        actions: (a) => [
          ...
          IconButton(icon: const Icon(Icons.info), onPressed: () => context.go('/authors/${a.id}')),
        ],
      );
    }
